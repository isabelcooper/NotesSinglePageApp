...
function expect(first) {
  ...
  function beInstanceOf(second) {
    (first instanceof second) ? loadResult(true) : loadResult(false)
  }

  return {toEqual: toEqual, toInclude: toInclude, beInstanceOf: beInstanceOf}
}
...
function loadResult(outcome) {
  ...
  document.getElementById('outcome').innerHTML += `<span style = "color: ${color};">${status}</span><br>`
}

function loadText(text) {
  document.getElementById('outcome').innerHTML += `<br><span style = "font-weight: bold"> ${text}<br></span>`
}
